...
class Info5 extends StatefulWidget {
  const Info5({Key? key, required this.name, }) : super(key: key);
  final String name;
                ...
                child: Padding(
                  ...
                  child: Align(
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          Navigator.of(context).push(MaterialPageRoute(builder: (context)=>Info6(name: widget.name)));
                          for(int i=0;i<24;i++){
                            if (!_btn[i]==true){
                              firestore.collection((_auth.currentUser)!.uid).doc("causelist$i").update({"data":cause[i]+1});
                              kind[_kindList[i]] = kind[_kindList[i]]+1;
                            }
                          }
                          firestore.collection((_auth.currentUser)!.uid).doc("trigger0").update({"data":kind[0]});
                          firestore.collection((_auth.currentUser)!.uid).doc("trigger1").update({"data":kind[1]});
                          firestore.collection((_auth.currentUser)!.uid).doc("trigger2").update({"data":kind[2]});
                          firestore.collection((_auth.currentUser)!.uid).doc("trigger3").update({"data":kind[3]});
                        },
                        style: ElevatedButton.styleFrom(
